...
<body style="padding-top:50px;">
    ...
    document.getElementById('appointmentForm').addEventListener('submit', function(event) {
        ...
        if (!appointmentDate || !appointmentTime) {
            errorMessage.textContent = 'Please select both appointment date and time.';
        ...
        // AJAX request to check if the appointment exists
        fetch("{{ route('check_appointment') }}", {
            method: "POST",
            headers: {
        ...
        .then(data => {
            if (data.exists) {
                errorMessage.textContent = 'This appointment has already been booked. Please choose a different time.';
            } else {
                // If the appointment does not exist, programmatically submit the form
